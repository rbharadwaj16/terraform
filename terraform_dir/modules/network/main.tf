...
locals {
  network_rg_name = toset([for i in data.azurerm_resource_group.network_rg: i.name])
  network_rg_location = toset([for i in data.azurerm_resource_group.network_rg: i.location])
}
